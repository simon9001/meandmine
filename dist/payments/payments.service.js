...
async function paystackRequest(path, method = 'GET', body) {
    ...
    if (!json.status)
        throw new UnprocessableError(json.message ?? 'Paystack error');
    return json.data;
}
...
export async function initializePayment(userId, orderId) {
    ...
    const amountKobo = Math.round(order.total_amount * 100);
    const data = await paystackRequest('/transaction/initialize', 'POST', {
        email,
        amount: amountKobo,
        currency: 'KES',
        reference: `ORD-${order.order_number}-${Date.now()}`,
        metadata: { order_id: orderId, user_id: userId, order_number: order.order_number },
...
export async function verifyPayment(reference, userId) {
    ...
    if (data.amount < expectedKobo)
        throw new UnprocessableError('Amount mismatch — possible fraud');
...
export async function handleWebhook(rawBody, signature) {
    ...
    if (event.event === 'charge.success') {
        ...
        const reportedKobo = event.data.amount;
        const expectedKobo = Math.round(payment.amount * 100);
...
export async function chargeMpesa(userId, orderId, phone) {
    const { data: order } = await supabaseAdmin
        .from('orders')
        .select('id, order_number, total_amount, payment_status, user_id')
        .eq('id', orderId)
...
export async function checkPaymentStatus(reference, userId) {
    ...
    const data = await paystackRequest(`/charge/${encodeURIComponent(reference)}`);
    if (data.status === 'success') {
        await handlePaymentConfirmed({
            paymentId: payment.id,
            orderId: payment.order_id,
...
export async function getPaymentForOrder(orderId, userId) {
