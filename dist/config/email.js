...
export async function sendEmail(payload) {
    const toAddresses = payload.to.map((r) => r.name ? `${r.name} <${r.email}>` : r.email);
...
export const templates = {
    ...
    orderConfirmed: (orderNumber, total, customerName = '') => ({
    ...
    paymentConfirmed: (orderNumber, total, customerName = '', items = [], delivery, discount, placedAt) => {
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;padding:32px 16px">
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
    ...
    adminOrderNotification: (orderNumber, total, customerName, customerEmail, customerPhone, items, delivery, discount) => {
        const fmt = (n) => `KES ${n.toLocaleString('en-KE', { minimumFractionDigits: 2 })}`;
        const subtotal = items.reduce((s, i) => s + i.price * i.quantity, 0);
        const deliveryLabel = delivery?.deliveryMethod === 'pickup' ? 'Self Pickup' : 'Home Delivery';
        const deliveryAddr = delivery
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;padding:32px 16px">
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
    ...
    orderStatusUpdate: (orderNumber, status, customerName = '') => ({
    ...
    orderDispatched: (orderNumber, customerName, items, dispatchInfo = {}) => ({
