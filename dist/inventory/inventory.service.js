...
export async function getInventory(productId, variantId) {
    let q = supabaseAdmin
        .from('inventory')
        .select('*')
...
export async function listLowStock(threshold) {
...
export async function adjustStock(productId, variantId, qty, reason) {
    ...
    let result;
    if (existing) {
        newStock = Math.max(0, existing.total_stock + qty);
...
export async function setStock(productId, variantId, totalStock, reorderPoint) {
    const { data, error } = await supabaseAdmin
        .from('inventory')
        .upsert({
        ...
        .single();
    if (error || !data)
        throw new BadRequestError(error?.message ?? 'Stock update failed');
    // Keep product_variants.stock_quantity in sync
    if (variantId) {
        await supabaseAdmin
            .from('product_variants')
...
export async function listAllInventory(_query) {
    ...
    for (const row of invRows ?? []) {
        invMap.set(`${row.product_id}:${row.variant_id ?? ''}`, row);
    }
    const result = [];
