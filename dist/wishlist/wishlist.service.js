...
export async function addToWishlist(userId, productId, variantId) {
    ...
    const { data, error } = await supabaseAdmin
        ...
        .select().single();
    if (error || !data)
        throw new BadRequestError(error?.message ?? 'Add to wishlist failed');
    return data;
}
...
export async function checkInWishlist(userId, productId) {
    const { data } = await supabaseAdmin
        .from('wishlists').select('id').eq('user_id', userId).eq('product_id', productId).maybeSingle();
    return { inWishlist: !!data, id: data?.id ?? null };
}
