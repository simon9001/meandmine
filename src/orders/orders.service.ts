...
export async function createOrder(userId: string, payload: CheckoutPayload, ip?: string) {
  // Idempotency check
...
export async function cancelOrder(userId: string, orderId: string, reason?: string) {
  ...
  if (!['pending', 'confirmed'].includes((order as { status: string }).status)) {
  ...
  const { data: order, error } = await supabaseAdmin
    ...
    .insert({
      ...
      shipping_address: {
        ...
        city:           payload.zone === 'nairobi' ? 'Nairobi' : 'Upcountry',
        country_code:   'KE',
      },
      subtotal,
