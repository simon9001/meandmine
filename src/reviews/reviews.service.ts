...
export async function listProductReviews(productId: string, query: { page?: string; limit?: string; rating?: string }) {
...
export async function createReview(userId: string, payload: {
...
export async function voteReview(userId: string, reviewId: string, vote: 'helpful' | 'not_helpful') {
  const { data: review } = await supabaseAdmin
    .from('reviews').select('id, user_id').eq('id', reviewId).single();
  if (!review) throw new NotFoundError('Review');
  if ((review as { user_id: string }).user_id === userId) throw new ForbiddenError('Cannot vote on your own review');
  ...
  if (existing) {
    if ((existing as { is_helpful: boolean }).is_helpful === isHelpful) {
...
export async function adminListReviews(query: { page?: string; limit?: string; status?: string }) {
  const { page, limit, offset } = parsePage(query);
  let q = supabaseAdmin
    .from('reviews')
